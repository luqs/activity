 ...
 */
package com.cuize.activity.web.util.oauth.pojo;
...
import com.cuize.activity.web.util.oauth.WeixinSupport;
import com.cuize.activity.web.util.oauth.http.HttpClient;
import com.cuize.activity.web.util.oauth.http.OAuth;
import com.cuize.activity.web.util.oauth.http.OAuth2Token;
import com.cuize.activity.web.util.oauth.http.Response;
...
public class OAuth2 extends WeixinSupport implements java.io.Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5372687869835237706L;
	/**
     * 公众号对象
     */
    ...
    public OAuth2Token login(String appId, String secret, String code, String grantType) throws WeixinException {
        ...
        logger.info("https://api.weixin.qq.com/sns/oauth2/access_token" + param);
        //根据请求结果判定，是否验证成功
        JSONObject jsonObj = res.asJSONObject();
        if (jsonObj != null) {
            if (Configuration.isDebug()) {
                logger.info("login返回json：" + jsonObj.toString());
            }
            ...
            if (obj != null) {
                ...
                oauth = new OAuth(appId, secret);
                //设置凭证
                this.oauth2Token = (OAuth2Token) JSON.parseObject(jsonObj.toJSONString(), OAuth2Token.class);
            }
        }
        return oauth2Token;
    }
    ...
    public OAuth2User getUserInfo(String lang) throws WeixinException {
        ...
        if (jsonObj != null) {
            if (Configuration.isDebug()) {
                logger.info("getUserInfo返回json：" + jsonObj.toString());
            }
            Object errcode = jsonObj.get("errcode");
            if (errcode != null) {
                //返回异常信息
            ...
            //设置公众号信息
            user = (OAuth2User) JSON.parseObject(jsonObj.toJSONString(), OAuth2User.class);
        }
        return user;
    }
}
